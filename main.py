...
import smart_trader
import settings
# TradeNotification is now safely imported because database.py is fixed
        ...
        if token and token != "SKIP_SESSION":
            try:
                ...
                if "Token is invalid" in str(e) and bot_active:
                    print("⚠️ Token Expired but Bot is Active (Race Condition Solved)"); login_state = "IDLE"
                else: raise e
        else:
    ...
    while True:
        try:
            if bot_active:
                ...
                except Exception as e: print(f"⚠️ Health Check Failed: {e}"); bot_active = False
...
def reset_connection():
    global bot_active, login_state; bot_active = False; login_state = "IDLE"
...
def callback():
    ...
    if t:
        try:
            ...
            kite.set_access_token(data["access_token"])
            bot_active = True; smart_trader.fetch_instruments(kite); flash("✅ System Online")
